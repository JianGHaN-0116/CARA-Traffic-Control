"""
Resource-constrained edge experiment.
Tests DRL performance under different edge resource levels.

The env now has resource-aware dynamics:
  - Low CPU makes Inspect slower and more expensive
  - Low bandwidth makes Mirror/Reroute more costly
  - Queue overload causes nonlinear latency growth
  - Recovery rate scales with resource_scale

Usage:
    python -m src.experiments.resource_constrained_exp [dataset]
"""
import os
import sys
import numpy as np
...
def evaluate_policy(env, policy, attack_threshold=0.84):
    ...
    queue_lengths, cpu_usages, resource_costs_list = [], [], []
    action_counts = {}
    done = False
    ...
    while not done:
        ...
        resource_costs_list.append(info["resource_cost"])
        action_counts[info["action_name"]] = action_counts.get(info["action_name"], 0) + 1
    ...
    metrics["avg_queue_length"] = float(np.mean(queue_lengths))
    metrics["avg_cpu_usage"] = float(np.mean(cpu_usages))
    metrics["avg_resource_cost"] = float(np.mean(resource_costs_list))
    metrics["total_steps"] = len(rewards)
    total_steps = max(len(rewards), 1)
    for action_name, count in action_counts.items():
        metrics[f"action_rate_{action_name.lower()}"] = count / total_steps
...
def main():
    ...
    results_dir = os.path.join(base_dir, "new_experiments", "resource_constrained", dataset)
    os.makedirs(results_dir, exist_ok=True)

    # Load DRL models
    drl_models = {}
    for alg in ["dqn", "ppo"]:
        model_path = os.path.join(model_results_dir, "seed_42", f"{alg}_edge_security_final.zip")
    ...
    # Resource scales (lower = more constrained)
    resource_configs = [
        (1.0, "Resource-rich (100%)"),
        (0.75, "Moderate (75%)"),
        (0.50, "Constrained (50%)"),
        (0.25, "Severe (25%)"),
    ]
    ...
    all_results = []
    for scale, label in resource_configs:
        print(f"\n{'='*60}")
        print(f"Resource level: {label}")
        print(f"{'='*60}")

        for policy_name, policy in policies.items():
            # Pass resource_scale to env so recovery rates and costs adapt
            ...
            metrics["resource_scale"] = scale
            metrics["resource_label"] = label
            ...
            print(f"  {policy_name}: F1={metrics.get('f1', 0):.4f} "
                  ...
                  f"PktLoss={metrics.get('avg_packet_loss', 0):.4f} "
                  f"Queue={metrics.get('avg_queue_length', 0):.4f}")

    os.makedirs(results_dir, exist_ok=True)
    df = pd.DataFrame(all_results)
    save_path = os.path.join(results_dir, "resource_constrained.csv")
