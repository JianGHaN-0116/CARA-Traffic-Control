"""
Export an enhanced controller-in-the-loop replay plan for Mininet validation.

Improvements over the original 119-step export:
  - 500+ replay steps (per_label=125, bins=5)
  - Includes PPO-TFC alongside NoControl, Greedy, CARA-TC, DQN-TFC
  - Outputs a plan CSV consumed by enhanced_replay.py
"""
import argparse
import os
import pickle
...
def select_replay_windows(windows, per_label=125, bins=5):
    records = []
    df = pd.DataFrame(
        {
...
def parse_args():
    ...
    parser.add_argument("--per-label", type=int, default=125)
...
def main():
    ...
    out_dir = args.out_dir or os.path.join(base_dir, "new_experiments", "enhanced_ovs_replay")
    ...
    print(f"Selected {len(replay_windows)} replay windows "
          f"(~{len(replay_windows)} steps per controller)")

    replay_window_path = os.path.join(out_dir, "edge_replay_windows_enhanced.pkl")
    with open(replay_window_path, "wb") as f:
        pickle.dump(replay_windows, f)
    manifest_df.to_csv(os.path.join(out_dir, "edge_replay_manifest_enhanced.csv"), index=False)

    rows = []

    print("Recording NoControl actions...")
    rows.extend(
    ...
    print("Recording Greedy actions...")
    rows.extend(
        record_policy_actions(
    ...
    print("Recording CARA-TC actions...")
    rows.extend(
        record_policy_actions(
            "CARA-TC",
            CARATCPolicy(
    ...
    print("Recording DQN-TFC actions...")
    dqn_seeds = [int(s) for s in args.dqn_seeds.split(",") if s.strip()]
    ...
    print("Recording PPO-TFC actions...")
    ppo_seeds = [int(s) for s in args.ppo_seeds.split(",") if s.strip()]
    ...
    plan_df.to_csv(os.path.join(out_dir, "enhanced_replay_plan.csv"), index=False)
    ...
    action_summary.to_csv(os.path.join(out_dir, "enhanced_action_summary.csv"), index=False)

    print(f"\nTotal plan rows: {len(plan_df)}")
    print(f"Controllers: {plan_df['controller'].unique().tolist()}")
    print(f"Steps per controller: {plan_df.groupby('controller')['replay_step'].nunique().to_dict()}")
    print(f"\nSaved enhanced replay plan to: {os.path.join(out_dir, 'enhanced_replay_plan.csv')}")
